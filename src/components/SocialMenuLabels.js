...
const SocialMenuLabels = props => {
    ...
    let color = props.screenWidth > props.canvasWidth * .55 ? 'rgb(255,255,255)' : `rgb(${1 + props.veilOpacity * 110},1,40)` //Change the color if the screen is so narrow that the otherwise white text is overlapping the moon. If the text does overlap the moon make the default color rgb(1,1,40) and increase the red according as a function of the veilOpacity so that the rgb(1,1,40) doesn't become camouflaged when the room darken to rgb(1,1,40)
    let narrowScreenRgbaOpacity = props.screenWidth > props.canvasWidth * .55 ? 0 : (props.veilOpacity - .3) * .5 //If the screen is so narrow that the text is overlapping the moon make the alpha channel of the rgba background no longer 0 but something commensurate with props.veilOpacity
    let left = props.canvasWidth < props.screenWidth ? props.margin + 60 : 60 //Make the parent div exactly 60px (which is the width of the social-menu-container) to the right of the left border of the canvas, or if the canvas is wider than the screen, 60px to the right of the left border of the screen.
